...
class AstroIoGame {
  constructor() {
    ...
    this.finalSize = 0;
    this.playerPoints = 0; // ← NEW: Track player points
...
setupTouchInput() {
    ...
    try {
      ...
      if (delta.players) {
        Object.entries(delta.players).forEach(([id, player]) => {
          ...
          // ← NEW: Track own player points
          if (id === this.myPlayerId && player.points !== undefined) {
  ...
  render() {
    ...
    if (this.myPlayerId && this.clientGameState.players[this.myPlayerId]) {
      ...
      // Actualizar Scale Panel
      this.ui.updateScalePanel(myPlayer.size);
      
      // ← NEW: Check and show cards based on points/size
